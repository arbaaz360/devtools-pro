...
function imageView(id: 'encoding.image-base64' | 'encoding.base64-image', label: string, operation: 'encode' | 'decode'): ToolView {
  return {
    ...
    savePresentation(manifest: ToolManifest, operationId, mime) { const jpeg = mime?.toLowerCase() === 'image/jpeg' || mime?.toLowerCase() === 'image/jpg'; const extension = jpeg ? 'jpg' : 'png'; return { ...standardSave(manifest, operationId, mime), suffix: operation === 'decode' ? `.decoded.${extension}` : '.base64.txt', label: operation === 'decode' ? 'Save decoded image…' : 'Save Base64…', filterName: operation === 'decode' ? `Image (${jpeg ? 'JPEG' : 'PNG'})` : 'Text', extensions: operation === 'decode' ? [extension] : ['txt'] }; },
  };
}
