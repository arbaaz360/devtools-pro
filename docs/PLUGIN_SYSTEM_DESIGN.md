...
8. Each tab retains its tool, inputs, options, results, and editor state. New creates an editable text tab; Open/import/drop targets the declared input or opens a new tab without losing a dirty document. Saved source files stay unchanged until an explicit save policy is applied.
