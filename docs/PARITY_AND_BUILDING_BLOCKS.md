...
`Ctrl+N` and **New document** create an empty UTF-8 tab. Small UTF-8 files can be edited in place. **Save** writes back to the file a tab came from (refusing if another program changed it since), **Save as** asks, and results are exported through the native host; see the save policy in PLUGIN_HOST_IMPLEMENTATION.md. Tabs can run independently, with a bounded global queue and revision tokens preventing late jobs from repainting a newer tab state. Selecting Image to Base64 shows a real bounded image preview and starts encoding automatically. Binary files no longer pass through a lossy text decoder, and incompatible tools explain the accepted input instead of producing junk output.
