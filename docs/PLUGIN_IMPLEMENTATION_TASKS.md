...
**Current state:** the native host now owns an injected `PluginHost` registry. Existing v1 tools are registered through a compatibility executor adapter, while native processors receive a host-owned `NativeExecutionContext` with named input ports, a bounded named artifact sink, options, cancellation, and progress. `run_tool` resolves manifests and executors through that registry, and accepted jobs return a typed execution identity. The generated bundled catalog is embedded during the Tauri build. JSON format/minify and URL encode/decode now use dedicated native plugin implementations through the same generic job/result path. JSON calls the shared streaming core, preserving source order and number lexemes. The registry has duplicate-id, native/legacy dispatch, deterministic ordering, bounded sink, and terminal-claim tests. Multi-document input binding (the compare workflow) and full v2 event/output descriptors remain in the next P03 slice.
