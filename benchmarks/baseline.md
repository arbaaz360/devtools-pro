...
- **Memory numbers are polled.** `PeakWorkingSet64` is read from the parent every ~20 ms in practice (the 5 ms interval is a lower bound; PowerShell adds overhead), so the shortest case received only 2–4 polls. The peak is monotonic, so a late poll still reports it, but anything allocated after the last poll is missed. Private bytes are sampled maxima (0.9–1.1 MiB), not true peaks.
